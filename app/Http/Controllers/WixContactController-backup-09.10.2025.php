...
class WixContactController extends Controller
{
    ...
    public function migrateAuto(Request $request)
    {
        ...
        // $fromToken = "3a2e5gyYg5ZYwmP7d-rsj5xz9Ha0bPfsNB0N8xLwuQ0.";
        // $toToken   = "44gPBfbbkmCLM9V5LPWPvZqY8Cv7AWpFoOmLRHEnoos.";
        ...
        $extractSrcLabelKeys = function (array $c): array {
            // handles: labelKeys (array of strings), labelKeys.items, info.labelKeys.items
        ...
        // ---------------------------------------------------------------------
        // Helper to deep-merge existing + incoming info
        // ---------------------------------------------------------------------
        $mergeInfo = function(array $existing, array $incoming): array {
            ...
            // Do NOT carry labelKeys or assignedUserIds here; we attach via dedicated patch after create/update
        ...
        foreach ($contacts as $src) {
            ...
            // ---------- Create or Update on TARGET (labels/assigned not in body) ----------
            $existing = $email ? $this->findContactByEmail($toToken, $email) : null;
            $targetContactId = null;

            if ($existing && !empty($existing['id'])) {
                $mergedInfo = $mergeInfo($existing['info'] ?? [], $filtered);
                $upd = $this->updateContactInWix($toToken, $existing['id'], $mergedInfo, $existing['revision'] ?? 1);
                $targetContactId = $existing['id'];
                if (!empty($upd['contact']['id'])) { $updated++; }
                else {
                    $failed++;
                    WixHelper::log('Auto Contacts Migration', 'Update failed: '.json_encode(['sent'=>$mergedInfo,'response'=>$upd]), 'warn');
                }
            } else {
                $created = $this->createContactInWix($toToken, $filtered);
                $targetContactId = $created['contact']['id'] ?? null;

                if ($targetContactId) { $imported++; }
                else {
                    $failed++;
        ...
        // ---------------------------------------------------------------------
        // PASS 2B: Map & PATCH assignedUserIds (added deeper diagnostics & fallback resolution)
        // ---------------------------------------------------------------------
        ...
        foreach ($pendingAssigned as $targetContactId => $items) {
            ...
            foreach ($items as $item) {
                ...
                // 5) Name-like string (no @ and not UUID) → we skip but log for clarity
    ...
    public function export(Request $request, WixStore $store)
    {
        ...
        WixHelper::log('Export Contacts', "Start: {$store->store_name} ({$fromStoreId}) [max={$max}, members=" . ($copyMembers?'1':'0') . ", attach=" . ($copyAttach?'1':'0') . "]", 'info');

        $accessToken = WixHelper::getAccessToken($fromStoreId);
        // $accessToken = "SRdp5VfHmARTGRd_q7IcwXMmk5u91di5rHyaUnL_P8E.";
        ...
        // -------- Query ALL labels once & build catalogs --------
        $queryAllLabels = function (string $token): array {
    ...
    public function import(Request $request, WixStore $store)
    {
        ...
        // $accessToken = "oBTrWmTRei-C8dewI64U-4uRR8WX3aBvvHZeZDdtKXw.";
        if (!$accessToken) {
        ...
        foreach ($contacts as $contact) {
            ...
            $targetContactId = null; $action = null;
            ...
            if ($existingContact && isset($existingContact['id'])) {
                ...
                if (!empty($upd['contact']['id'])) { $updatedContacts++; $action = 'updated'; }
                else {
                ...
                if (isset($result['contact']['id'])) { $targetContactId = $result['contact']['id']; $importedContacts++; $action = 'created'; }
                else {
